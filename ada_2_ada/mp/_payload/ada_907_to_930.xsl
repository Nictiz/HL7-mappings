...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns:nfa2a="http://www.nictiz.nl/functions/ada2ada" xmlns:nf="http://www.nictiz.nl/functions" xmlns:uuid="http://www.uuid.org" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema">
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="newAdaXmlRefId" as="element()?">
            <xsl:apply-templates select="*" mode="addReferenceId"/>
        </xsl:variable>
        <xsl:apply-templates select="$newAdaXmlRefId" mode="ada907_2_930"/>
    </xsl:template>
    ...
    <xsl:template match="adaxml" mode="ada907_2_930">
        <xsl:copy>
            <xsl:apply-templates select="@*" mode="#current"/>
            <xsl:attribute name="xsi:noNamespaceSchemaLocation">../../../9.3.0/<xsl:value-of select="data/*/local-name()"/>/ada_schemas/ada_<xsl:value-of select="data/*/local-name()"/>.xsd</xsl:attribute>
    ...
    <xsl:template match="adaxml/data/*" mode="ada907_2_930">
        <xsl:copy>
            <!-- bestaande attributen kopiëren -->
            <xsl:apply-templates select="@*" mode="#current"/>
            <!-- app en formName attribuut alsnog overschrijven -->
            <xsl:attribute name="app">mp-mp93</xsl:attribute>
            <xsl:variable name="mapTrans" select="$mapTransaction[@usecase = local-name(current())]/mp930"/>
                ...
                <bouwstenen>
                    ...
                    <xsl:for-each-group select=".//informant/persoon[rol_of_functie[@code = '2'][@codeSystem = '2.16.840.1.113883.2.4.3.11.60.20.77.5.4']]" group-by="adaextension/reference_id/@value">
                        ...
                        <zorgaanbieder id="{$zaId}">
                            ...
                            <!-- adresgegevens not in 907 either, but required in 930, see MP-407 -->
                            <adresgegevens>
                                <straat/>
                                <huisnummer/>
                                <postcode/>
    ...
    <xsl:template match="medicatieafspraak" mode="ada907_2_930">
        <xsl:copy>
            <xsl:apply-templates select="@*" mode="#current"/>
            <xsl:apply-templates select="identificatie | afspraakdatum" mode="#current"/>
            <!-- gebruiksperiode -->
            <!-- zijn er gegevens voor gebruiksperiode, structuur is anders in 9.3.0 -->
    ...
    <xd:doc>
        <xd:desc>Gebruiksinstructie/HerhaalperiodeCyclischSchema/@unit=dag (930) Gebruiksinstructie/HerhaalperiodeCyclischSchema/@unit=d (907) </xd:desc>
    </xd:doc>
    <xsl:template match="herhaalperiode_cyclisch_schema" mode="ada907_2_930">
        <xsl:copy>
            <xsl:apply-templates select="@*" mode="#current"/>
    ...
    <xsl:template match="verstrekkingsverzoek[not(@conceptId) or @conceptId = '2.16.840.1.113883.2.4.3.11.60.20.77.2.3.19963']" mode="ada907_2_930">
        <xsl:copy>
            <xsl:apply-templates select="@*" mode="#current"/>
    ...
    <xsl:template match="relaties_ketenzorg" mode="ada907_2_930">
    ...
    <xsl:template match="toedieningsafspraak" mode="ada907_2_930">
        <xsl:copy>
            <xsl:apply-templates select="@*" mode="#current"/>
            <xsl:apply-templates select="identificatie | afspraakdatum" mode="#current"/>
            <!-- gebruiksperiode -->
    ...
    <xsl:template match="toedieningsafspraak/aanvullende_informatie" mode="ada907_2_930">
    ...
    <xsl:template match="toedieningsafspraak/reden_afspraak" mode="ada907_2_930">
    ...
    <xsl:template match="verstrekking/aanvullende_informatie" mode="ada907_2_930">
        <medicatieverstrekking_aanvullende_informatie>
            <xsl:apply-templates select="@* | node()" mode="ada907_2_930"/>
        </medicatieverstrekking_aanvullende_informatie>
    </xsl:template>
    ...
    <xsl:template match="verstrekking" mode="ada907_2_930">
    ...
    <xsl:template match="medicatie_gebruik" mode="ada907_2_930">
        <medicatiegebruik>
            <xsl:apply-templates select="@*" mode="#current"/>
    ...
    <xsl:template match="gebruiksperiode_start | gebruiksperiode_eind | gebruiksperiode" mode="ada907_2_930"/>

    <xd:doc>
        <xd:desc>Handle toedieningsschema, order of elements. We do have ada schema with is_flexibel, but in different order </xd:desc>
    </xd:doc>
    <xsl:template match="toedieningsschema" mode="ada907_2_930">
        <xsl:copy>
            <xsl:apply-templates select="@*" mode="#current"/>
    ...
    <xsl:template match="toedientijd" mode="ada907_2_930">
        <xsl:copy>
            <xsl:apply-templates select="@*" mode="#current"/>
    ...
    <xsl:template match="verstrekkingsverzoek/verbruiksperiode" mode="ada907_2_930">
        <xsl:copy>
            <xsl:apply-templates select="@*" mode="#current"/>
    ...
    <xsl:template match="lichaamslengte | lichaamsgewicht" mode="ada907_2_930"/>

    <xd:doc>
        <xd:desc>sturen_medicatievoorschrift, TODO: adapt for 930</xd:desc>
    </xd:doc>
    <xsl:template match="sturen_medicatievoorschrift" mode="ada907_2_930">
        <xsl:copy>
            <!-- attributen kopiëren -->
            <xsl:apply-templates select="@*" mode="#current"/>
    ...
    <xd:doc>
        <xd:desc> datum van 9.0.7 naar 9 3.0 </xd:desc>
    </xd:doc>
    <!--<xsl:template match="(medicatieafspraak | toedieningsafspraak)/afspraakdatum" mode="ada907_2_930">-->
    <!-- Match expression was not XSLT2 compliant. Changed to: -->
    <xsl:template match="medicatieafspraak/afspraakdatum | toedieningsafspraak/afspraakdatum" mode="ada907_2_930">
    ...
    <xsl:template match="verstrekkingsverzoek/datum" mode="ada907_2_930">
        <xsl:element name="{concat(local-name(..), '_datum_tijd')}">
            <xsl:apply-templates select="@* | node()" mode="#current"/>
        </xsl:element>
    </xsl:template>
    ...
    <xsl:template match="verstrekking/aanschrijfdatum" mode="ada907_2_930">
    ...
    <xsl:template match="doseerinstructie" mode="ada907_2_930">
        <xsl:copy>
    ...
    <xsl:template match="criterium/code" mode="ada907_2_930">
        <criterium>
            <xsl:apply-templates select="@* | node()" mode="#current"/>
        </criterium>
    </xsl:template>
    ...
    <xsl:template match="toedieningsduur" mode="ada907_2_930">
        <xsl:copy>
            <tijds_duur>
                <xsl:apply-templates select="@* | node()" mode="#current"/>
            </tijds_duur>
    ...
    <xsl:template match="verstrekking/datum" mode="ada907_2_930">
    ...
    <xsl:template match="medicatie_gebruik/registratiedatum" mode="ada907_2_930">
    ...
    <xsl:template match="medicatie_gebruik/gerelateerde_afspraak/identificatie_medicatieafspraak" mode="ada907_2_930">
        <identificatie>
            <xsl:apply-templates select="@* | node()" mode="#current"/>
        </identificatie>
    </xsl:template>
    ...
    <!-- Match expression was not XSLT2 compliant. Changed to: -->
    <xsl:template match="medicatieafspraak/stoptype | toedieningsafspraak/stoptype | medicatie_gebruik/stoptype" mode="ada907_2_930">
    ...
    <xsl:template match="medicatieafspraak/geannuleerd_indicator" mode="ada907_2_930">
        <xsl:choose>
            <xsl:when test="@value = 'true'">
                <medicatieafspraak_stop_type code="89925002" codeSystem="2.16.840.1.113883.6.96" displayName="geannuleerd"/>
            </xsl:when>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="toedieningsafspraak/geannuleerd_indicator" mode="ada907_2_930">
        <xsl:choose>
            <xsl:when test="@value = 'true'">
                <toedieningsafspraak_stop_type code="89925002" codeSystem="2.16.840.1.113883.6.96" displayName="geannuleerd"/>
            </xsl:when>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="voorschrijver | verstrekkingsverzoek/auteur | auteur_is_zorgverlener | informant_is_zorgverlener" mode="ada907_2_930">
        <xsl:copy>
    ...
    <xsl:template match="zorgverlener/naamgegevens[naamgegevens] | zorgverlener/zorgverlener_naam[naamgegevens] | zorgaanbieder[not(parent::zorgverlener)][zorgaanbieder]" mode="ada907_2_930">
        <xsl:apply-templates select="node()" mode="#current"/>
    </xsl:template>
    ...
    <xsl:template match="documentgegevens/auteur/auteur_is_zorgaanbieder" mode="ada907_2_930">
        <xsl:copy>
            <xsl:apply-templates select="@* | node()" mode="#current"/>
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:template match="documentgegevens/auteur/auteur_is_zorgaanbieder/zorgaanbieder[not(zorgaanbieder_identificatienummer/(@value | @nullFlavor))]" mode="ada907_2_930">
        <xsl:copy>
            <xsl:apply-templates select="@*" mode="#current"/>
            <zorgaanbieder_identificatienummer nullFlavor="NI"/>
            <xsl:apply-templates select="node()" mode="#current"/>
    ...
    <!--<xsl:template match="(verstrekkingsverzoek/beoogd_verstrekker | toedieningsafspraak/verstrekker | verstrekking/verstrekker | zorgverlener/zorgaanbieder)[zorgaanbieder] | auteur_is_zorgaanbieder[not(ancestor::documentgegevens)]" mode="ada907_2_930">-->
    <!-- Match expression was not XSLT2 compliant. Changed to: -->
    <xsl:template match="verstrekkingsverzoek/beoogd_verstrekker[zorgaanbieder] | toedieningsafspraak/verstrekker[zorgaanbieder] | verstrekking/verstrekker[zorgaanbieder] | zorgverlener/zorgaanbieder[zorgaanbieder] | auteur_is_zorgaanbieder[not(ancestor::documentgegevens)]" mode="ada907_2_930">
        <xsl:copy>
    ...
    <xsl:template match="zorgverlener/zorgaanbieder[not(zorgaanbieder)]" mode="ada907_2_930">
        <xsl:copy>
    ...
    <xsl:template match="afgesproken_geneesmiddel | te_verstrekken_geneesmiddel | geneesmiddel_bij_toedieningsafspraak | verstrekt_geneesmiddel" mode="ada907_2_930">
        <xsl:copy>
    ...
    <xsl:template match="gebruiks_product" mode="ada907_2_930">
        <gebruiksproduct>
            <farmaceutisch_product datatype="reference" value="{product/adaextension/reference_id/@value}"/>
        </gebruiksproduct>
    </xsl:template>
    ...
    <xsl:template match="informant/persoon" mode="ada907_2_930">
        <!-- special handling for thuiszorg, because in 930 a thuiszorg professional is conveyed using zorgverlener see issue https://bits.nictiz.nl/browse/ZIB-1075 -->
        <xsl:choose>
    ...
    <xsl:template match="rol_of_functie" mode="ada907_2_930">
        <rol>
            <xsl:apply-templates select="@*" mode="#current"/>
    ...
    <xsl:template match="ingredient" mode="ada907_2_930">
        <xsl:copy>
    ...
    <xsl:template match="hoeveelheid_ingredient" mode="ada907_2_930">
    ...
    <xsl:template match="hoeveelheid_product" mode="ada907_2_930">
        <product_hoeveelheid>
            <xsl:apply-templates select="@* | node()" mode="#current"/>
        </product_hoeveelheid>
    </xsl:template>
    ...
    <xsl:template match="gerelateerde_afspraak[*]" mode="ada907_2_930">
    ...
    <xsl:template match="relatie_naar_afspraak_of_gebruik[*]" mode="ada907_2_930">
    ...
    <xsl:template match="gerelateerde_verstrekking[*]" mode="ada907_2_930">
        <xsl:for-each select="identificatie">
    ...
    <xsl:template match="aantal/min | waarde/min" mode="ada907_2_930">
        <minimum_waarde>
            <xsl:apply-templates select="@* | node()" mode="#current"/>
        </minimum_waarde>
    </xsl:template>
    ...
    <xsl:template match="aantal/vaste_waarde | waarde/vaste_waarde" mode="ada907_2_930">
        <nominale_waarde>
            <xsl:apply-templates select="@* | node()" mode="#current"/>
        </nominale_waarde>
    </xsl:template>
    ...
    <xsl:template match="aantal/max | waarde/max" mode="ada907_2_930">
        <maximum_waarde>
            <xsl:apply-templates select="@* | node()" mode="#current"/>
        </maximum_waarde>
    </xsl:template>
    ...
    <xsl:template match="zorgverlener_identificatie_nummer" mode="ada907_2_930">
    ...
    <xsl:template match="zorgaanbieder_identificatie_nummer" mode="ada907_2_930">
    ...
    <xd:doc>
        <xd:desc>non supported nor needed stuff in MP9 3.0 </xd:desc>
    </xd:doc>
    <xsl:template match="patient-afko" mode="ada907_2_930"/>
    ...
    <xsl:template match="@* | node()" mode="ada907_2_930 addReferenceId">
        <xsl:copy>
            <xsl:apply-templates select="@* | node()" mode="#current"/>
        </xsl:copy>
    </xsl:template>
