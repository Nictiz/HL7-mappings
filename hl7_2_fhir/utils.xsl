...
<xsl:stylesheet xmlns:uuid="java.util.UUID" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:hl7="urn:hl7-org:v3" xmlns:hl7nl="urn:hl7-nl:v3" xmlns:f="http://hl7.org/fhir" xmlns:local="urn:fhir:stu3:functions" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" exclude-result-prefixes="#all" version="2.0">
    ...
    <xd:doc>
        <xd:desc>Tries to create an as stable as possible URI from and hl7:id element. urn:oid:@root.@extension if @root and @extension is numeric. urn:uuid:@extension if extension is uuid. urn:uuid:@root if @root is uuid. urn:uuid:randomUUID() otherwise.</xd:desc>
        <xd:param name="id"/>
    ...
    <xsl:function name="local:getUriFromId" as="xs:string">
        <xsl:param name="id" as="element(hl7:id)*"/>
        ...
        <xsl:choose>
            <xsl:when test="$id[not(@root = $maskIdentifiers)][matches(@root, $OIDpattern)][matches(@extension, '^\d+$')]">
                ...
                <xsl:value-of select="concat('urn:oid:', $ii/string-join((@root, @extension),'.'))"/>
            </xsl:when>
            ...
            <xsl:otherwise>
                <xsl:value-of select="concat('urn:uuid:', uuid:randomUUID())"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
    <xd:doc>
    ...
    <xsl:template name="EN-to-HumanName" as="element()*">
        ...
        <xsl:for-each select="$in[not(@nullFlavor)]">
            <name xmlns="http://hl7.org/fhir">
                ...
                <xsl:if test="hl7:family">
                    <family value="{hl7:family | hl7:prefix[tokenize(@qualifier,'\s') = 'VV']}">
                        <xsl:for-each select="hl7:prefix[tokenize(@qualifier,'\s') = 'VV'][tokenize(@qualifier,'\s') = 'BR'] | 
                            hl7:prefix[tokenize(@qualifier,'\s') = 'VV'][tokenize(@qualifier,'\s') = 'SP'] | 
                            ...
                            <xsl:choose>
                                ...
                                <xsl:when test="hl7:prefix[tokenize(@qualifier,'\s') = 'VV'][tokenize(@qualifier,'\s') = 'SP']">
                                    <extension url="http://hl7.org/fhir/StructureDefinition/partnername-own-prefix">
                                        <valueString value="{.}"/>
                                    </extension>
                                </xsl:when>
                                <xsl:when test="hl7:family[tokenize(@qualifier,'\s') = 'BR']">
                                    <extension url="http://hl7.org/fhir/StructureDefinition/humanname-partner-name">
                                        <valueString value="{.}"/>
                                    </extension>
                                </xsl:when>
                                <xsl:when test="hl7:family[tokenize(@qualifier,'\s') = 'SP']">
                                    <extension url="http://hl7.org/fhir/StructureDefinition/humanname-partner-name">
                                        <valueString value="{.}"/>
                                    </extension>
                                </xsl:when>
