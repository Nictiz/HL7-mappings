...
<xsl:stylesheet exclude-result-prefixes="#all" xmlns="urn:hl7-org:v3" xmlns:nf="http://www.nictiz.nl/functions" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" xmlns:f="http://hl7.org/fhir"  xmlns:hl7="urn:hl7-org:v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema">
    ...
    <xsl:template match="hl7:ControlActProcess">
        ...
        let $gRvpToestemmingClient          := format-dateTime($gPatientDOB-dt + xs:dayTimeDuration('P12Y'),'[Y0001][M01][D01]')
        let $gRvpToestemmingGezaghebbende   := format-dateTime($gPatientDOB-dt + xs:dayTimeDuration('P3M'),'[Y0001][M01][D01]')
        let $gContact2Wekentm6Maanden       := format-dateTime($gPatientDOB-dt + xs:dayTimeDuration('P5M'),'[Y0001][M01][D01]')
        let $gContact12tm18Jaar             := format-dateTime($gCREATIONDATETIME-dt - xs:dayTimeDuration('P1Y'),'[Y0001][M01][D01]')
        let $gDktpVaccinatie-1              := format-dateTime($gPatientDOB-dt + xs:dayTimeDuration('P3M'),'[Y0001][M01][D01]')
        let $gDktpVaccinatieReactieStart-1  := format-dateTime($gPatientDOB-dt + xs:dayTimeDuration('P3M1D'),'[Y0001][M01][D01]')
        let $gDktpVaccinatieReactieEind-1   := format-dateTime($gPatientDOB-dt + xs:dayTimeDuration('P3M5D'),'[Y0001][M01][D01]')
        let $gDktpVaccinatieReactieGemeld-1 := format-dateTime($gPatientDOB-dt + xs:dayTimeDuration('P3M5D'),'[Y0001][M01][D01]')
        let $gDktpVaccinatie-2              := format-dateTime($gPatientDOB-dt + xs:dayTimeDuration('P5M3D'),'[Y0001][M01][D01]')
        let $gMenAcwyVaccinatie-1           := format-dateTime($gCREATIONDATETIME-dt - xs:dayTimeDuration('P2M'),'[Y0001][M01][D01]')
        let $gATermeDatum                   := format-date($gPatientDOB-dt + xs:dayTimeDuration('P7D'),'[Y0001][M01][D01]')
    ...
    <!-- Organisatie ID -->
    <xsl:template match="//hl7:CareProvisionRequest[hl7:templateId/@root='2.16.840.1.113883.2.4.6.10.100.10000']/hl7:author/hl7:assignedEntity/hl7:id">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:attribute name="extension">{$organizationId}</xsl:attribute>
            <xsl:apply-templates select="node()"/>
    ...
    <xsl:template match="//hl7:careProvisionEvent/hl7:id">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:attribute name="root">2.16.528.1.1007.3.3.{if ($organizationId castable as xs:integer) then string(xs:integer($organizationId))</xsl:attribute>
            <xsl:apply-templates select="node()"/>
        </xsl:copy>
    </xsl:template>
    ...
    <!-- organisatie ID -->
    <xsl:template match="//hl7:careProvisionEvent/hl7:assignedEntity[hl7:templateId/@root='2.16.840.1.113883.2.4.6.10.100.122']/hl7:id">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:attribute name="extension">{$organizationId}</xsl:attribute>
            <xsl:apply-templates select="node()"/>
