                    ...
                    <xsl:attribute name="{$attrName}">
                        <xsl:choose>
                            ...
                            <xsl:when test="count($attrConcepts[@type = 'item']) ge 1">
                                <xsl:text>{</xsl:text>
                                <xsl:value-of select="string-join($attrConcepts/@shortName, ' | ')"/>
                ...
                <xsl:choose>
                    <xsl:when test="$elemNameLocal = 'buildingNumberSuffix'">
                        <xsl:element name="xsl:if">
                            <xsl:attribute name="test" select="string-join($elemConcepts[@type = 'item']/@shortName, ' | ')"/>
                            <xsl:element name="{if ($elemPrefixNS = 'urn:hl7-org:v3') then $elemNameLocal else $elemName}" namespace="{$elemPrefixNS}">
                                <xsl:element name="xsl:value-of">
                                    <xsl:attribute name="select">
                                        ...
                                        <xsl:value-of select="string-join($elemConcepts[@type = 'item']/@shortName, ' | ')"/>
                                        <xsl:text>, ' ')</xsl:text>
                                    </xsl:attribute>
                                </xsl:element>
                            </xsl:element>
                    ...
                    <xsl:otherwise>
                        <xsl:element name="xsl:for-each">
                            <xsl:attribute name="select" select="string-join($elemConcepts[@type = 'item']/@shortName, ' | ')"/>
            ...
            <xsl:when test="@contains">
                <xsl:choose>
                    <xsl:when test="$elemConcepts | $containedElemConcepts">
                        <xsl:element name="xsl:for-each">
                            <xsl:attribute name="select" select="string-join($elemConcepts/@shortName | $containedElemConcepts/@shortName, ' | ')"/>
                            <xsl:element name="{if ($elemPrefixNS = 'urn:hl7-org:v3') then $elemNameLocal else $elemName}" namespace="{$elemPrefixNS}">
                ...
                <xsl:element name="xsl:when">
                    <xsl:choose>
                        ...
                        <xsl:when test="self::element">
                            ...
                            <xsl:choose>
                                <xsl:when test="$elemConcepts | $containedElemConcepts">
                                    <xsl:attribute name="test" select="string-join($elemConcepts/@shortName | $containedElemConcepts/@shortName, ' | ')"/>
                                </xsl:when>
                                <xsl:otherwise>
                        ...
                        <xsl:when test="self::include">
                            ...
                            <xsl:choose>
                                <xsl:when test="$elemConcepts">
                                    <xsl:attribute name="test" select="string-join($elemConcepts/@shortName, ' | ')"/>
                                </xsl:when>
                                <xsl:otherwise>
