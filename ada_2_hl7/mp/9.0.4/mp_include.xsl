...
<xsl:stylesheet xmlns="urn:hl7-org:v3" xmlns:hl7="urn:hl7-org:v3" xmlns:hl7nl="urn:hl7-nl:v3"
   ...
   <xsl:template name="make_UCUM2Gstandard_translation">
      ...
      <xsl:choose>
         ...
         <!-- TODO: hack in 9.04 om de in ADA forms aangepaste eenheden goede HL7 genereert. -->
         <!-- De officiele UCOM unit is 1, maar via het formulier zou evt. ook eenheid, stuk, tablet of dosis binnen kunnen komen -->
         <xsl:when
            test="($UCUMunit eq '1') or (lower-case($UCUMunit) eq 'eenheid') or (lower-case($UCUMunit) eq 'tablet') or
                  (lower-case($UCUMunit) eq 'stuk') or (lower-case($UCUMunit) eq 'dosis')">
   ...
   <xsl:function name="nf:convertUnit_ADA2UCUM" as="xs:string">
      ...
      <xsl:choose>
         <xsl:when test="lower-case($ADAtime) eq 'eenheid'">1</xsl:when>
         <xsl:when test="lower-case($ADAtime) eq 'stuk'">1</xsl:when>
         <xsl:when test="lower-case($ADAtime) eq 'dosis'">1</xsl:when>
         <xsl:when test="lower-case($ADAtime) eq 'tablet'">1</xsl:when>
         <xsl:when test="lower-case($ADAtime) eq 'druppel'">[drp]</xsl:when>
         <xsl:otherwise>
   ...
   <xsl:function name="nf:convertTime_UCUM2HL7" as="xs:string?">
      ...
      <xsl:if test="$UCUMtime">
         <xsl:choose>
            ...
            <xsl:when test="$UCUMtime eq 'minuut'">min</xsl:when>
            ...
            <xsl:when test="$UCUMtime eq 'min'">min</xsl:when>
            <xsl:when test="$UCUMtime eq 'm'">min</xsl:when>
            ...
            <xsl:when test="./aantal/vaste_waarde/@value">
               <center>
                  <xsl:call-template
                     ...
                     <xsl:with-param name="UCUMunit"
                        select="if (./eenheid/@unit) then nf:convertUnit_ADA2UCUM(./eenheid/@unit) else
                        (if (./eenheid/@value) then nf:convertUnit_ADA2UCUM(./eenheid/@value) else
                        (if (./eenheid/@displayName) then nf:convertUnit_ADA2UCUM(./eenheid/@displayName) else ''))"/>
                  </xsl:call-template>
               </center>
            </xsl:when>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9064_20160601000000">
      ...
      <numerator>
         <xsl:call-template
            ...
            <xsl:with-param name="UCUMunit" select="if (./eenheid/@unit) then nf:convertUnit_ADA2UCUM(./eenheid/@unit) else
               (if (./eenheid/@value) then nf:convertUnit_ADA2UCUM(./eenheid/@value) else
               (if (./eenheid/@displayName) then nf:convertUnit_ADA2UCUM(./eenheid/@displayName) else ''))"/>
         </xsl:call-template>
      </numerator>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9071_20160618204153">
      ...
      <xsl:for-each select="./hoeveelheid_ingredient">
         <numerator xsi:type="PQ">
            ...
            <!-- TODO pub 9.04 patch waarbij eenheden in verschillende attributen kunnen zitten -->
            <xsl:variable name="UCUMunit" select="if (./eenheid/@unit) then nf:convertUnit_ADA2UCUM(./eenheid/@unit) else
               (if (./eenheid/@value) then nf:convertUnit_ADA2UCUM(./eenheid/@value) else
               (if (./eenheid/@displayName) then nf:convertUnit_ADA2UCUM(./eenheid/@displayName) else ''))"/>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9076_20160619200644">
      ...
      <xsl:if test="(../../../herhaalperiode_cyclisch_schema) and (../toedieningsduur)">
         <error>
            Herhaalperiode cyclisch schema en toedienduur worden niet samen ondersteund.
         </error>
      </xsl:if>
      <xsl:choose>
         <!-- Cyclisch schema -->
         ...
         <xsl:otherwise>
            <xsl:choose>
               ...
               <xsl:when
                  test="count(./weekdag) = 1 and count(./toedientijd) &lt;= 1 and not(./frequentie[tijdseenheid/@value != 'week'])">
               ...
               <xsl:when
                  test="count(./weekdag) > 1 or (./weekdag and count(./frequentie[tijdseenheid/@value != 'week']) >= 1) or (./weekdag and count(./toedientijd) > 1)">
                  <xsl:variable name="aantalPerWeek">  <!-- Zou onzin moeten zijn om meerdere keren per week te combineren met weekdagen -->
                     <xsl:value-of select="./frequentie[tijdseenheid/@value = 'week'][1]/aantal/vaste_waarde/@value" />
                  </xsl:variable>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9079_20160620162955">
      ...
      <hl7nl:frequency>
         ...
         <hl7nl:denominator xsi:type="hl7nl:PQ">
            ...
            <!-- Tijdelijke fix 9.04: bij interval staat de unit in @unit, terwijl op de andere plekken waarop 
                 makeTimeDenominatorAttribs wordt aangeroepen, @value gebruikt moet worden. -->
            <xsl:attribute name="value" select="1"/>
            <xsl:attribute name="unit" select="nf:convertTime_UCUM2HL7(./@unit)"/>
         </hl7nl:denominator>
      </hl7nl:frequency>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9087_20160621125157">
      ...
      <xsl:for-each select="$vv">
         <supply classCode="SPLY" moodCode="RQO">
            ...
            <code codeSystemName="Medicatieproces acts" displayName="Verstrekkingsverzoek" 
               code="3" codeSystem="2.16.840.1.113883.2.4.3.11.60.20.77.5.3"/>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9131_20160714202241">
      ...
      <supply classCode="SPLY" moodCode="PRP">
         ...
         <xsl:for-each select="//voorstelgegevens/toelichting">
            <entryRelationship typeCode="SUBJ" inversionInd="true">
               <xsl:call-template
                  name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9183_20170818085324"/>
            </entryRelationship>
         </xsl:for-each>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9149_20160725134340">
      ...
      <substanceAdministration classCode="SBADM" moodCode="RQO">
         ...
         <xsl:if test="./zo_nodig/criterium">
            <precondition>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9166_20170516000000">
      ...
      <substanceAdministration classCode="SBADM" moodCode="EVN">
         ...
         <code code="16076005" codeSystem="2.16.840.1.113883.6.96" codeSystemName="SNOMED Clinical Terms"
      ...
      <xsl:for-each select="$ma">
         ...
         <xsl:for-each select="./relatie_naar_afspraak_of_gebruik">
            ...
            <xsl:for-each select="./identificatie_23289">
               ...
               <entryRelationship typeCode="REFR">
                  <xsl:call-template
                     name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9176_20170522183626">
                     <xsl:with-param name="identificatieElement" select="."/>
                  </xsl:call-template>
               </entryRelationship>
            </xsl:for-each>
