...
<xsl:stylesheet exclude-result-prefixes="#all" xmlns:nf="http://www.nictiz.nl/functions" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:pharm="urn:ihe:pharm:medication" xmlns:hl7="urn:hl7-org:v3" xmlns:hl7nl="urn:hl7-nl:v3" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xd:doc>
        <xd:desc>Dit is een conversie van MP 9.1.0 naar ADA 9.1 sturen_voorstel_verstrekkingsverzoek</xd:desc>
    </xd:doc>
    <xsl:output method="xml" indent="yes"/>
    <!-- de xsd variabelen worden gebruikt om de juiste conceptId's te vinden voor de ADA xml -->
    <xsl:param name="schema" select="document('../ada_schemas/sturen_voorstel_verstrekkingsverzoek.xsd')"/>

    <xsl:variable name="templateId-voorstel-verstrekkingsverzoek" select="'2.16.840.1.113883.2.4.3.11.60.20.77.10.9131'"/>
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="medicatiegegevens-lijst-90" select="//hl7:organizer[hl7:code[@code = '104'][@codeSystem = '2.16.840.1.113883.2.4.3.11.60.20.77.4']]"/>
        <xsl:call-template name="sturen_voorstel_verstrekkingsverzoek">
    ...
    <xsl:template name="sturen_voorstel_verstrekkingsverzoek">
        <xsl:param name="in" select="//hl7:organizer[hl7:code[@code = '104'][@codeSystem = '2.16.840.1.113883.2.4.3.11.60.20.77.4']]"/>
        <xsl:call-template name="doGeneratedComment">
            <xsl:with-param name="in" select="$in/ancestor::*[hl7:ControlActProcess]"/>
        </xsl:call-template>
        <adaxml xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="../ada_schemas/ada_sturen_voorstel_verstrekkingsverzoek.xsd">
            ...
            <data>
                <xsl:for-each select="$in">
                    ...
                    <sturen_voorstel_verstrekkingsverzoek app="mp-mp910" shortName="sturen_voorstel_verstrekkingsverzoek" formName="voorstel_verstrekkingsverzoek" transactionRef="2.16.840.1.113883.2.4.3.11.60.20.77.4.104" transactionEffectiveDate="2016-03-23T16:54:50" prefix="mp-" language="nl-NL" title="Generated" id="TODO">
                        ...
                        <xsl:for-each select="$patient">
                            <xsl:variable name="schemaFragment" select="nf:getADAComplexType($schema, nf:getADAComplexTypeName($schema, 'sturen_voorstel_verstrekkingsverzoek'))"/>
                        ...
                        <xsl:variable name="component" select=".//*[hl7:templateId/@root = ($templateId-voorstel-verstrekkingsverzoek)]"/>
                            ...
                            <xsl:element name="{$elemName}">
                                ...
                                <xsl:element name="{$elemName}">
                                    ...
                                    <xsl:element name="{$elemName}">
                                        ...
                                        <xsl:for-each select="current-group()[hl7:templateId/@root = $templateId-voorstel-verstrekkingsverzoek]">
                                            <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9131_20160714201445">
                                                <xsl:with-param name="in" select="."/>
                                                <xsl:with-param name="schema" select="$schema"/>
                                                <xsl:with-param name="schemaFragment" select="$schemaFragment"/>
                                            </xsl:call-template>
