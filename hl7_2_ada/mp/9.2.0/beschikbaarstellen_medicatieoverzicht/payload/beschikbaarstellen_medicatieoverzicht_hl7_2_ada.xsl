...
<xsl:stylesheet exclude-result-prefixes="#all" xmlns:nf="http://www.nictiz.nl/functions" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:pharm="urn:ihe:pharm:medication" xmlns:hl7="urn:hl7-org:v3" xmlns:hl7nl="urn:hl7-nl:v3" xmlns:f="http://hl7.org/fhir" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xd:doc>
        <xd:desc>Dit is een conversie voor MP 9 2.0 van HL7v3 naar ADA beschikbaarstellen medicatieoverzicht</xd:desc>
    </xd:doc>
    ...
    <xsl:param name="schemaFileString" as="xs:string?">../../hl7_2_ada/mp/9.2.0/beschikbaarstellen_medicatieoverzicht/ada_schemas/beschikbaarstellen_medicatieoverzicht.xsd</xsl:param>
    ...
    <xsl:variable name="medicatieoverzicht-lijst-92" select="//hl7:organizer[hl7:code[@code = '129'][@codeSystem = '2.16.840.1.113883.2.4.3.11.60.20.77.4']] | //hl7:organizer[hl7:code[@code = '419891008'][@codeSystem = '2.16.840.1.113883.6.96']] | hl7:ClinicalDocument[hl7:code[@code = '52981000146104'][@codeSystem = '2.16.840.1.113883.6.96']]"/>
    ...
    <xsl:param name="theId">
        <xsl:choose>
            ...
            <xsl:when test="string-length($medicatieoverzicht-lijst-92/../../../hl7:id/@extension) gt 0">
                <!-- let's use the extension of the message id -->
                <xsl:value-of select="$medicatieoverzicht-lijst-92/../../../hl7:id/@extension"/>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="/">
        <xsl:call-template name="medicatieoverzicht-92-ADA">
            <xsl:with-param name="medicatieoverzicht-lijst" select="$medicatieoverzicht-lijst-92"/>
        </xsl:call-template>
    </xsl:template>

    <xd:doc>
        <xd:desc>Handles HL7 9 2.0 medication information, transforms it to ada.</xd:desc>
        <xd:param name="medicatieoverzicht-lijst">HL7 9 2.0 organizer/CDA with medication information.</xd:param>
    </xd:doc>
    <xsl:template name="medicatieoverzicht-92-ADA">
        <xsl:param name="medicatieoverzicht-lijst" select="$medicatieoverzicht-lijst-92"/>
            ...
            <adaxml xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
                ...
                <data>
                    <xsl:for-each select="$medicatieoverzicht-lijst">
                        ...
                        <beschikbaarstellen_medicatieoverzicht app="mp-mp920" shortName="beschikbaarstellen_medicatieoverzicht" formName="medicatieoverzicht" transactionRef="2.16.840.1.113883.2.4.3.11.60.20.77.4.191" transactionEffectiveDate="2021-04-14T15:39:26" prefix="mp-" language="nl-NL" title="{$theId}" id="{$theId}">
                            ...
                            <documentgegevens>
                                ...
                                <xsl:for-each select="hl7:effectiveTime[@value]">
                                    <xsl:call-template name="handleTS">
                                        <xsl:with-param name="elemName">document_datum</xsl:with-param>
                                    </xsl:call-template>
                                </xsl:for-each>
                                ...
                                <verificatie_patient>
                                    ...
                                    <xsl:for-each select="hl7:participant[@typeCode = 'VRF'][(hl7:participantRole | hl7:associatedEntity)[@classCode = 'PAT']]/hl7:time">
                                        <xsl:call-template name="handleTS">
                                            <xsl:with-param name="elemName">verificatie_datum</xsl:with-param>
                                     </xsl:call-template>
                                    </xsl:for-each>
                                </verificatie_patient>
