This is a preliminary version of converting Medicatievoorschrift 9.0.6 payload to a relatively simple dataset xml format (based on functional definition of a MP 9 voorschrift transaction). It is based on the publication with timestamp 26-04-2018 14:09 (http://decor.nictiz.nl/medicatieproces/mp-html-20180426T140905/project.html).
...
//substanceAdministration[code/@code='16076005']
...
The folder "sturen_medicatievoorschrift/wrapper contains the stylesheet that does the conversion to help recreate VZVZ wrappers (for LSP) when converting from ada to a format exchanged on the LSP: sturen_medicatievoorschrift_wrapper.xsl. It outputs an xml with the necessary parameters for wrappers.

The folder "sturen_medicatievoorschrift/payload" contains the stylesheet that does the conversion for the payload: "sturen_medicatievoorschrift_906_to_ADA.xsl"
...
The folder 'hl7_instance' contains sample ada instance files. They are in no way functionally valid (i.e. the situation can probably not be validated in real life). The folder ada_instance contains the result of the conversion for these sample files.

The ADA user front-end for mp-mp9 can be found here: 
