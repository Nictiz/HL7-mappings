...
=Addenda voor inhoudelijke gegevens=</xsl:text>
    ...
    <xd:doc>
        <xd:desc> only geboortedatum is present in demografische_gegevens in kernset 2.2, so no reason to nest the geboortedatum under group 'demografische_gegevens'</xd:desc>
        <xd:param name="level"/>
    </xd:doc>
    <xsl:template match="demografische_gegevens" mode="maak-tabel-rij">
        <xsl:param name="level" select="xs:int(1)"/>
    ...
    <xd:doc>
        <xd:desc> only zorginstelling_lvrid is present in zorginstelling in kernset 2.2, so no reason to nest the zorginstelling_lvrid under group 'zorginstelling'</xd:desc>
        <xd:param name="level"/>
    </xd:doc>
