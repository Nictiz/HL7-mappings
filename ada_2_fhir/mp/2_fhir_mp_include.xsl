...
<xsl:stylesheet exclude-result-prefixes="#all" xmlns="http://hl7.org/fhir" xmlns:f="http://hl7.org/fhir" xmlns:local="urn:fhir:stu3:functions" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:nf="http://www.nictiz.nl/functions" xmlns:uuid="http://www.uuid.org" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xsl:variable name="products" as="element()*">
        ...
        <xsl:for-each-group select="//product" group-by="nf:getProductGroupingKey(./product_code)">
            <xsl:for-each-group select="current-group()" group-by="nf:getGroupingKeyDefault(.)">
                ...
                <xsl:variable name="search-mode">include</xsl:variable>
                <uniek-product xmlns="">
                    <group-key>
                        <xsl:value-of select="current-grouping-key()"/>
                    </group-key>
                            ...
                            <resource>
                                <xsl:choose>
                                    <xsl:when test="$referById">
                                        ...
                                        <xsl:call-template name="zib-Product-612">
                                            <xsl:with-param name="medication-id" select="$fhir-resource-id"/>
                                        </xsl:call-template>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:apply-templates select="." mode="doMedication612"/>
                                    </xsl:otherwise>
                                </xsl:choose>
                            </resource>
                        </entry>
                                ...
                                <xsl:with-param name="observation-id">
                                    <xsl:choose>
                                        <xsl:when test="$referById">
                                            ...
                                            <xsl:variable name="patient-ref" select="nf:getFullUrlOrId('Patient', nf:getGroupingKeyPatient($ada-patient), true())"/>
                            ...
                            <xsl:apply-templates select="." mode="doConditionForProbleem">
                                <xsl:with-param name="condition-id">
                                    <xsl:choose>
                                        <xsl:when test="$referById">
                                            <xsl:variable name="ada-patient" select="./ancestor::*[ancestor::data]/patient"/>
                                            <xsl:variable name="patient-reference" select="nf:getFullUrlOrId('Patient', nf:getGroupingKeyPatient($ada-patient), true())"/>
    ...
    <xsl:template name="patient-payload">
        ...
        <xsl:for-each select="naamgegevens[.//(@value | @code | @nullFlavor)]">
            <xsl:call-template name="nl-core-humanname-2.0">
                <xsl:with-param name="in" select="."/>
                <xsl:with-param name="unstructured-name" select="ongestructureerde_naam"/>
            </xsl:call-template>
        </xsl:for-each>
        <!-- contactgegevens -->
    ...
    <xsl:template name="zib-Product-612" match="product" mode="doMedication612">
        <xsl:param name="medication-id" as="xs:string?"/>
        <Medication xmlns="http://hl7.org/fhir">
            ...
            <meta>
                <profile value="http://nictiz.nl/fhir/StructureDefinition/mp612-DispenseToFHIRConversion-Product"/>
            </meta>
            <xsl:call-template name="medication-payload"/>
        </Medication>
    </xsl:template>
    ...
    <xd:doc>
        <xd:desc>Helper template for medication payload independent of version (6.12 or nl-core).</xd:desc>
    </xd:doc>
    <xsl:template name="medication-payload">
