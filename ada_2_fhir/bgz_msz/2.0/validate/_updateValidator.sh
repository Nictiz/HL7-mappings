#!/bin/bash
pubsource=https://github.com/hapifhir/org.hl7.fhir.core/releases/latest/download/
...
if ! type "curl" > /dev/null; then
	echo "ERROR: Script needs curl to download latest IG Validator. Please install curl."
	exit 1
fi
...
if test -f "$publisher" ; then
	echo "IG Validator FOUND in input-cache"
	...
	upgrade=true
	if test -f "$publisher"; then
		echo "IG Validator FOUND in parent folder"
		jarlocation="$publisher"
		jarlocationname="Parent Folder"
		upgrade=true
	else
		echo "IG Validator NOT FOUND in input-cache or parent folder"
...
if [[ $skipPrompts == true ]] || [[ $response =~ ^[yY].*$ ]]; then
	...
	echo "Downloading most recent validator to $jarlocationname - it's ~100 MB, so this may take a bit"
...
if [[ $skipPrompts == true ]] || [[ $response =~ ^[zZ].*$ ]]; then
