...
<xsl:stylesheet exclude-result-prefixes="#all" xmlns="http://hl7.org/fhir" xmlns:f="http://hl7.org/fhir" xmlns:local="urn:fhir:stu3:functions" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:nf="http://www.nictiz.nl/functions" xmlns:uuid="http://www.uuid.org" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xsl:template name="payerReference" match="treatment_directive[not(@datatype = 'reference')][.//(@value | @code | @nullFlavor)]" mode="doPayerReference-2.0">
        <xsl:variable name="theIdentifier" select="identificatie_nummer[@value] | identification_number[@value]"/>
        <xsl:variable name="theGroupKey" select="nf:getGroupingKeyDefault(.)"/>
        <xsl:variable name="theGroupElement" select="$payers[group-key = $theGroupKey]" as="element()?"/>
        <xsl:choose>
    ...
    <xd:doc>
        <xd:desc>Produces a FHIR entry element with a Coverage resource for Payer</xd:desc>
        ...
        <xd:param name="fhirResourceId">Optional. Value for the entry.resource.Coverage.id</xd:param>
        <xd:param name="searchMode">Optional. Value for entry.search.mode. Default: include</xd:param>
    </xd:doc>
    <xsl:template name="payerEntry" match="treatment_directive[not(@datatype = 'reference')][.//(@value | @code | @nullFlavor)]" mode="doPayerEntry-2.0" as="element(f:entry)">
