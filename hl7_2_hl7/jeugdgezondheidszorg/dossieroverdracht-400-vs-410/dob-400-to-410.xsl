...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:hl7="urn:hl7-org:v3" xmlns:exslt="http://exslt.org/common" exclude-result-prefixes="xs xd hl7 xsi exslt" version="1.0">
    <xd:doc scope="stylesheet">
        <xd:desc>
            ...
            <xd:p><xd:b>Purpose:</xd:b> Conversie van een valide Dossieroverdrachtbericht op basis van BDS 4.0.2 (REPC_IN902120NL03) naar een valide versie op basis van BDS 4.1.0 (REPC_IN902120NL03). Regelt zelf het bijwerken van de weergavenamen bij rubrieken, groepen en elementen. Laat de weergavenamen van waarden ongemoeid.</xd:p>
            <xd:p>
                <xd:b>History:</xd:b>
            </xd:p>
            <xd:ul>
...
<xsl:variable name="W0639_HL7_W0646_HL7_W0647_HL7">
    ...
    <xsl:template match="hl7:value[@codeSystem = '2.16.840.1.113883.2.4.4.40.8']" mode="dob402">
        <xsl:copy>
            <xsl:choose>
                <!-- Aangeboren afwijking: 12, Text: Aangeboren afwijking -->
                <!-- De weergavenamen worden niet aangepast -->
                <!--<xsl:when test="@code = '12'">
                    <xsl:copy-of select="@code"/>
                    <xsl:copy-of select="@codeSystem"/>
                    <xsl:attribute name="displayName">
                        <xsl:value-of select="'Erfelijke aangeboren afwijking'"/>
                    </xsl:attribute>
                </xsl:when>-->
                <!-- Inserted a dummy 'when' for the commented code -->
                <xsl:when test="false()">
                    <xsl:apply-templates select="@* | node()" mode="dob402"/>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xd:doc>
        <xd:desc>BDS-rubrieknamen, BDS-groepnamen en BDS-elementnamen bijwerken conform 4.1.0</xd:desc>
    </xd:doc>
