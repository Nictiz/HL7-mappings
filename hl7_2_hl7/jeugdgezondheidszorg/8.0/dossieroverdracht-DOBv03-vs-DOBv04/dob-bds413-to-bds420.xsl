...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:util="urn:hl7:utilities" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:hl7="urn:hl7-org:v3" xmlns:exslt="http://exslt.org/common" exclude-result-prefixes="xs xd hl7 xsi exslt" version="1.0">
    ...
    <xd:doc>
        <xd:desc>Rubriek 31: elementen 268, 269, 815, 1437, 1438, 1439 vervallen</xd:desc>
    </xd:doc>
    <xsl:template match="hl7:component[hl7:observation[hl7:code[@code = '268' or @code = '269' or @code = '815' or @code = '1437' or @code = '1438' or @code = '1439'][@codeSystem = '2.16.840.1.113883.2.4.4.40.267']]]" mode="dob400">
        <xsl:comment><xsl:text> element </xsl:text><xsl:value-of select="hl7:observation/hl7:code/@code"/><xsl:text> </xsl:text><xsl:value-of select="hl7:observation/hl7:code/@displayName"/><xsl:text> is vervallen </xsl:text></xsl:comment>
    </xsl:template>
    ...
    <xsl:template match="hl7:pertinentInformation[hl7:rubricCluster/hl7:code/@code = 'R038']" mode="dob400">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:comment><xsl:text> Rubriek 38 is verwijderd omdat het enkele vervallen elementen bevat.</xsl:text></xsl:comment>
                <xsl:apply-templates select="@* | ./hl7:rubricCluster/hl7:component" mode="dob400"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="hl7:pertinentInformation[hl7:rubricCluster/hl7:code/@code = 'R042']" mode="dob400">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:comment><xsl:text> Rubriek 42 is verwijderd omdat het enkele vervallen elementen bevat.</xsl:text></xsl:comment>
                <xsl:apply-templates select="@* | ./hl7:rubricCluster/hl7:component" mode="dob400"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
