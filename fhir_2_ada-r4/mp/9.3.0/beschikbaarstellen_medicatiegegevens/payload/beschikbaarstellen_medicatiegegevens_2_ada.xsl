...
-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:util="urn:hl7:utilities" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:f="http://hl7.org/fhir" xmlns:local="urn:fhir:stu3:functions" xmlns:nf="http://www.nictiz.nl/functions" exclude-result-prefixes="#all" version="2.0">
    ...
    <xsl:template match="/">
        <xsl:variable name="bouwstenen" as="element()*">
            ...
            <!-- zorgaanbieder as MedicationUse author needs to end up in zorgaanbieder in ada. Not all Location resources are zorgaanbieders, may also be afleverlocatie ... -->
            <xsl:for-each select="f:Bundle/f:entry/f:resource/(f:MedicationUse | f:MedicationStatement)/f:extension[@url = $urlExtMedicationUseAuthor]/f:valueReference">
                <xsl:apply-templates select="nf:resolveRefInBundle(.)/f:Location" mode="nl-core-HealthcareProvider-Organization"/>
            </xsl:for-each>
        </xsl:variable>
        ...
        <adaxml xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="../ada_schemas/ada_beschikbaarstellen_medicatiegegevens.xsd">
            ...
            <data>
                <beschikbaarstellen_medicatiegegevens app="mp-mp93" shortName="beschikbaarstellen_medicatiegegevens" formName="medicatiegegevens" transactionRef="2.16.840.1.113883.2.4.3.11.60.20.77.4.374" transactionEffectiveDate="2022-06-30T00:00:00" prefix="mp-" language="nl-NL">
