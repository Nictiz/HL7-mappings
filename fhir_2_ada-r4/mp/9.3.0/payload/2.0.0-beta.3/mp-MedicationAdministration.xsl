...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:f="http://hl7.org/fhir" xmlns:local="urn:fhir:stu3:functions" xmlns:nf="http://www.nictiz.nl/functions" xmlns:util="urn:hl7:utilities" exclude-result-prefixes="#all" version="2.0">
	...
	<xsl:variable name="urlMpMedicationAdministration"><xsl:value-of select="$urlBaseNictizProfile"/>mp-MedicationAdministration</xsl:variable>

	<xd:doc>
		<xd:desc>Template to convert f:MedicationAdministration to ADA medicatietoediening</xd:desc>
	</xd:doc>
	<xsl:template match="f:MedicationAdministration" mode="mp-MedicationAdministration">
		<medicatietoediening>
			<!-- identificatie -->
	...
	<xsl:template match="f:identifier" mode="mp-MedicationAdministration">
	...
	<xsl:template match="f:medicationReference" mode="mp-MedicationAdministration">
	...
	<xsl:template match="f:dosage/f:extension[@url = $urlExtMedicationAdministration2AgreedAmount]/f:valueQuantity" mode="mp-MedicationAdministration">
		<afgesproken_hoeveelheid>
	...
	<xsl:template match="f:dosage/f:site/f:text" mode="mp-MedicationAdministration">
	...
	<xsl:template match="f:effectiveDateTime" mode="mp-MedicationAdministration">
	...
	<xsl:template match="f:performer" mode="mp-MedicationAdministration">
...
<!--						<xsl:variable name="resourceProvider" select="nf:resolveRefInBundle(($resource/f:PractitionerRole/(f:organization|f:location)[f:reference])[1])"/>-->
						<zorgaanbieder>
							<!-- MP9 3.0 beta2 it was possible to convey zorgaanbieder -->
					...
					<!-- pre MP9 3.0 it was possible to convey zorgverlener -->
					<!-- MP9 3 beta3 it's possible to convey zorgverlener again -->
	...
	<xsl:template match="f:request" mode="mp-MedicationAdministration">
	...
	<xsl:template match="f:supportingInformation" mode="mp-MedicationAdministration">
		<relatie_toedieningsafspraak>
			<xsl:call-template name="Reference-to-identificatie"/>
		</relatie_toedieningsafspraak>
	</xsl:template>
	...
	<xsl:template match="f:extension[@url = $urlExtMedicationAdministration2AgreedDateTime]" mode="mp-MedicationAdministration">
