...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:f="http://hl7.org/fhir" xmlns:local="urn:fhir:stu3:functions" xmlns:nf="http://www.nictiz.nl/functions" xmlns:util="urn:hl7:utilities" exclude-result-prefixes="#all" version="2.0">
    ...
    <xsl:template match="f:MedicationDispense" mode="mp-AdministrationAgreement">
        <toedieningsafspraak>
            <!-- identificatie -->
            ...
            <!-- MP-1176 aanvullende_informatie nolonger supported in MP9.3.0.0 beta.3 -->
            ...
            <xsl:apply-templates select="f:extension[@url = $urlExtRelationAdministrationAgreement]" mode="#current"/>
    ...
    <xsl:template match="f:identifier" mode="mp-AdministrationAgreement">
    ...
    <xsl:template match="f:extension[@url = $extAdministrationAgreementDateTime]" mode="mp-AdministrationAgreement">
    ...
    <xsl:template match="f:extension[@url = $urlExtAdministrationAgreementAdditionalInformation]" mode="mp-AdministrationAgreement">
    ...
    <xsl:template match="f:modifierExtension[@url = $urlExtStoptype]" mode="mp-AdministrationAgreement">
    ...
    <xsl:template match="f:status" mode="mp-AdministrationAgreement">
    ...
    <xsl:template match="f:extension[@url = $extAdministrationAgreementAgreementReason]" mode="mp-AdministrationAgreement">
    ...
    <xsl:template match="f:extension[@url = $extAdministrationAgreementModificationDiscontinuationReason]" mode="mp-AdministrationAgreement">
        <xsl:for-each select="f:valueCodeableConcept">
            <xsl:call-template name="CodeableConcept-to-code">
    ...
    <xsl:template match="f:extension[@url = $urlExtRelationAdministrationAgreement]" mode="mp-AdministrationAgreement">
    ...
    <xsl:template match="f:authorizingPrescription" mode="mp-AdministrationAgreement">
