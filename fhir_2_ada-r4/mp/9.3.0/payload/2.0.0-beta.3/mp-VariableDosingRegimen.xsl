...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:f="http://hl7.org/fhir" xmlns:local="urn:fhir:stu3:functions" xmlns:nf="http://www.nictiz.nl/functions" xmlns:util="urn:hl7:utilities" exclude-result-prefixes="#all" version="2.0">
	...
	<xsl:template match="f:MedicationRequest" mode="mp-VariableDosingRegimen">
	...
	<xsl:template match="f:requester" mode="mp-VariableDosingRegimen">
		<auteur>
	...
	<xsl:template match="f:extension[@url = 'http://nictiz.nl/fhir/StructureDefinition/ext-MedicationAgreement.MedicationAgreementAdditionalInformation']" mode="mp-VariableDosingRegimen">
	...
	<xsl:template match="f:medicationReference" mode="mp-VariableDosingRegimen">
		<afgesproken_geneesmiddel>
	...
	<xsl:template match="f:authoredOn" mode="mp-VariableDosingRegimen">
	...
	<xsl:template match="f:reasonCode" mode="mp-VariableDosingRegimen">
		<xsl:call-template name="CodeableConcept-to-code">
			<xsl:with-param name="in" select="."/>
	...
	<xsl:template match="f:note" mode="mp-VariableDosingRegimen">
