...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:f="http://hl7.org/fhir" xmlns:local="urn:fhir:stu3:functions" xmlns:nf="http://www.nictiz.nl/functions" exclude-result-prefixes="#all" version="2.0">
        ...
        <xsl:for-each select="$in">
            <xsl:choose>
                <xsl:when test="f:dosage | f:dosageInstruction">
                    <xsl:apply-templates select="f:dosage | f:dosageInstruction" mode="mp-InstructionsForUse"/>
                </xsl:when>
                <!-- only omschrijving -->
    ...
    <xsl:template match="f:dosageInstruction | f:dosage" mode="mp-InstructionsForUse">
    ...
    <xsl:template match="f:additionalInstruction" mode="mp-InstructionsForUse">
    ...
    <xsl:template match="f:modifierExtension[@url = $ext-InstructionsForUse-RepeatPeriodCyclicalSchedule]" mode="mp-InstructionsForUse">
    ...
    <xsl:template match="f:route" mode="mp-InstructionsForUse">
        <xsl:call-template name="CodeableConcept-to-code">
            <xsl:with-param name="in" select="."/>
    ...
    <xsl:template match="f:sequence" mode="mp-InstructionsForUse">
    ...
    <xsl:template match="f:boundsDuration" mode="mp-InstructionsForUse">
        <xsl:call-template name="Duration-to-hoeveelheid">
    ...
    <xsl:template match="f:doseQuantity | f:extension[@url = $urlExtMedicationAdministration2AgreedAmount]/f:valueQuantity" mode="mp-InstructionsForUse">
    ...
    <xsl:template match="f:doseRange" mode="mp-InstructionsForUse">
    ...
    <xsl:template match="f:timing" mode="mp-InstructionsForUse">
        <toedieningsschema>
    ...
    <xsl:template match="f:repeat" mode="mp-InstructionsForUse">
    ...
    <xsl:template match="f:duration" mode="mp-InstructionsForUse">
        <toedieningsduur>
    ...
    <xsl:template match="f:frequency" mode="mp-InstructionsForUse">
        <xsl:choose>
    ...
    <xsl:template match="f:frequencyMax" mode="mp-InstructionsForUse">
    ...
    <xsl:template match="f:periodUnit" mode="mp-InstructionsForUse">
    ...
    <xsl:template match="f:period" mode="mp-InstructionsForUse">
    ...
    <xsl:template match="f:timeOfDay" mode="mp-InstructionsForUse">
    ...
    <xsl:template match="f:maxDosePerPeriod" mode="mp-InstructionsForUse">
        <maximale_dosering>
    ...
    <xsl:template match="f:rateRatio" mode="mp-InstructionsForUse">
    ...
    <xsl:template match="f:rateRange" mode="mp-InstructionsForUse">
