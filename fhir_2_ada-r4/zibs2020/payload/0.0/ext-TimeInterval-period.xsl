...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:f="http://hl7.org/fhir" xmlns:local="urn:fhir:stu3:functions" xmlns:nf="http://www.nictiz.nl/functions" exclude-result-prefixes="#all" version="2.0">
    ...
    <xsl:template match="f:extension[@url = ($urlExtTimeInterval-Period, $urlExtTimeIntervalPeriod)]" mode="urlExtTimeInterval-Period">
        <xsl:apply-templates select="f:valuePeriod" mode="urlExtTimeInterval-Period"/>
    </xsl:template>

    <xd:doc>
        <xd:desc>Template to resolve f:valuePeriod</xd:desc>
    </xd:doc>
    <xsl:template match="f:valuePeriod" mode="urlExtTimeInterval-Period">
    ...
    <xsl:template match="f:start" mode="urlExtTimeInterval-Period">
    ...
    <xsl:template match="f:end" mode="urlExtTimeInterval-Period">
        <xsl:call-template name="datetime-to-datetime">
