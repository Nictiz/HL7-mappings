    ...
    <xsl:template name="Reference-to-identificatie">
        ...
        <xsl:choose>
            <xsl:when test="f:reference">
                <xsl:variable name="referenceValue" as="xs:string" select="current()/f:reference/@value"/>
                <!-- https://hl7.org/fhir/R4/references.html#regex -->
              <!--  <xsl:variable name="restRegex" as="xs:string">((http|https)://([A-Za-z0-9\-\\\.:%\$]*/)+)?(Account|ActivityDefinition|AdverseEvent|AllergyIntolerance|Appointment|AppointmentResponse|AuditEvent|Basic|Binary|BiologicallyDerivedProduct|BodyStructure|Bundle|CapabilityStatement|CarePlan|CareTeam|CatalogEntry|ChargeItem|ChargeItemDefinition|Claim|ClaimResponse|ClinicalImpression|CodeSystem|Communication|CommunicationRequest|CompartmentDefinition|Composition|ConceptMap|Condition|Consent|Contract|Coverage|CoverageEligibilityRequest|CoverageEligibilityResponse|DetectedIssue|Device|DeviceDefinition|DeviceMetric|DeviceRequest|DeviceUseStatement|DiagnosticReport|DocumentManifest|DocumentReference|EffectEvidenceSynthesis|Encounter|Endpoint|EnrollmentRequest|EnrollmentResponse|EpisodeOfCare|EventDefinition|Evidence|EvidenceVariable|ExampleScenario|ExplanationOfBenefit|FamilyMemberHistory|Flag|Goal|GraphDefinition|Group|GuidanceResponse|HealthcareService|ImagingStudy|Immunization|ImmunizationEvaluation|ImmunizationRecommendation|ImplementationGuide|InsurancePlan|Invoice|Library|Linkage|List|Location|Measure|MeasureReport|Media|Medication|MedicationAdministration|MedicationDispense|MedicationKnowledge|MedicationRequest|MedicationStatement|MedicinalProduct|MedicinalProductAuthorization|MedicinalProductContraindication|MedicinalProductIndication|MedicinalProductIngredient|MedicinalProductInteraction|MedicinalProductManufactured|MedicinalProductPackaged|MedicinalProductPharmaceutical|MedicinalProductUndesirableEffect|MessageDefinition|MessageHeader|MolecularSequence|NamingSystem|NutritionOrder|Observation|ObservationDefinition|OperationDefinition|OperationOutcome|Organization|OrganizationAffiliation|Patient|PaymentNotice|PaymentReconciliation|Person|PlanDefinition|Practitioner|PractitionerRole|Procedure|Provenance|Questionnaire|QuestionnaireResponse|RelatedPerson|RequestGroup|ResearchDefinition|ResearchElementDefinition|ResearchStudy|ResearchSubject|RiskAssessment|RiskEvidenceSynthesis|Schedule|SearchParameter|ServiceRequest|Slot|Specimen|SpecimenDefinition|StructureDefinition|StructureMap|Subscription|Substance|SubstanceNucleicAcid|SubstancePolymer|SubstanceProtein|SubstanceReferenceInformation|SubstanceSourceMaterial|SubstanceSpecification|SupplyDelivery|SupplyRequest|Task|TerminologyCapabilities|TestReport|TestScript|ValueSet|VerificationResult|VisionPrescription)/[A-Za-z0-9\-\.]{1,64}(/_history/[A-Za-z0-9\-\.]{1,64})?</xsl:variable>-->
                
                <!-- LR relatief absoluut -->
                <xsl:variable name="reference" as ="xs:string">
                    <xsl:choose>
                       
                        <!-- checks whether the reference is a relative url by using the regex provided by the FHIR R4 doc https://hl7.org/fhir/R4/references.html#regex -->
                        <xsl:when test="matches($referenceValue, $restRegex) and replace($referenceValue, $restRegex, '$1') = ''">
                            <xsl:variable name="entryFullUrl" select="string(ancestor::f:entry/f:fullUrl/@value)"/>
                            
                            <!-- check if the fullUrl for the bundle entry containing the resource is a RESTful one (see the RESTful URL regex) -->
                            <xsl:choose>
                            <xsl:when test="matches($entryFullUrl, $restRegex)">
                                <xsl:variable name="baseUrl" select="replace($entryFullUrl, $restRegex, '$1')"/>
                                <xsl:value-of select="string(concat($baseUrl, $referenceValue))" />
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:call-template name="util:logMessage">
                                    ...
                                    <xsl:with-param name="msg">
                                        ...
                                        <xsl:text> the fullUrl for the bundly entry containing the resource should be a RESTful one (see: https://hl7.org/fhir/R4/references.html#regex). Therefore information will be lost.</xsl:text>
                                    </xsl:with-param>
                                </xsl:call-template>
                            </xsl:otherwise>
                            </xsl:choose>
                ...
                <xsl:variable name="nonFilteredResource" select="ancestor::f:Bundle/f:entry[f:fullUrl/@value = $reference]/f:resource" as="element()*"/>
                ...
                <xsl:choose>
                    ...
                    <!-- references to resources with identifier -->
                    ...
                    <!-- references to resources without an identifier (such as Medication, PractitionerRole) -->
                    <xsl:when test="$resource">
                        <xsl:attribute name="value" select="nf:convert2NCName($reference)"/>
