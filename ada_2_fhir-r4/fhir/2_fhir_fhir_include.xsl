...
<xsl:stylesheet exclude-result-prefixes="#all" xmlns="http://hl7.org/fhir"
    xmlns:f="http://hl7.org/fhir" xmlns:uuid="http://www.uuid.org"
    xmlns:local="urn:fhir:stu3:functions" xmlns:nf="http://www.nictiz.nl/functions"
    xmlns:nm="http://www.nictiz.nl/mappings" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl"
    xmlns:xs="http://www.w3.org/2001/XMLSchema"
    xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:util="urn:hl7:utilities" version="2.0">

    <xsl:import href="../../ada_2_fhir/fhir/2_fhir_fhir_include.xsl"/>

    <xd:doc>
        <xd:desc> When $populateId equals true(), Resource.id will be filled. It will be filled with
            @logicalId if it is present in the ada-element or generated if no @logicalId is present.
            When $populateId equals false(), Resource.id will not be filled. If $referincingStrategy
            equals 'logicalId', the value of $populateId will be ignored. </xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>$referencingStrategy will be filled with one of the following values: <xd:ul>
                ...
                <xd:li>uuid</xd:li>
                <xd:li>none</xd:li>
            </xd:ul> When $referencingStrategy equals 'logicalId', the value of $populateId is
            ignored. A Resource.id is added to the resource, with its value being populated from (in
            this order) @logicalId on the root of the ada element being referenced or from a
            template with mode '_generateId'. It is the responsibility of the use case XSLT to
            extract the fullUrl from $fhirMetadata. Should not be used when there is no FHIR server
            available to retrieve the resources. When $referencingStrategy equals 'uuid', all
            referencing is done using uuids. It is the responsibility of the use case XSLT to
            extract the fullUrl from $fhirMetadata. Meant for use within Bundles. Be sure to include
            all referenced resources in the Bundle! When $referencingStrategy equals 'none', it is
            attempted to generate a Reference from an identifier being present in the referenced
            ada-element. If this is not possible, referencing fails. With any strategy, the
            referencing of a specific ada element can be overruled by adding an @referenceUri to the
            ada reference element. The value of @referenceUri is added to Reference.reference (for
            example an absolute url or URN). An attempt is made to parse a fullUrl form
            @referenceUri within $fhirMetadata. </xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>Mapping between ADA scenario names and the resulting FHIR resource type and profile
            id's. Note that that muliple nm:map elements with the same ada attribute might occur if
            an ADA scenario results in multiple profiles. Note that that muliple nm:map elements
            with different ada attribute but same other attributes might occur due to differences in
            ada datasets. </xd:desc>
    </xd:doc>
    <xsl:variable name="ada2resourceType">
        ...
        <nm:map ada="allergie_intolerantie" resource="AllergyIntolerance"
        ...
        <nm:map ada="farmaceutisch_product" resource="Medication"
        ...
        <nm:map ada="medicatie_contra_indicatie" resource="Flag"
        ...
        <nm:map ada="medicatie_toediening" resource="MedicationAdministration"
            profile="mp-MedicationAdministration2"/>
        <nm:map ada="medicatietoediening" resource="MedicationAdministration"
            profile="mp-MedicationAdministration2"/>
        <nm:map ada="medicatieafspraak" resource="MedicationRequest"
            profile="mp-MedicationAgreement"/>
        <nm:map ada="medicatieverstrekking" resource="MedicationDispense"
            profile="mp-MedicationDispense"/>
        <nm:map ada="medisch_hulpmiddel" resource="DeviceUseStatement"
        ...
        <nm:map ada="sturen_medicatievoorschrift" resource="Bundle"
            profile="mp-MedicationPrescription-Bundle"/>
        <nm:map ada="sturen_afhandeling_medicatievoorschrift" resource="Bundle"
            profile="mp-MedicationPrescriptionProcessing-Bundle"/>
        <nm:map ada="sturen_antwoord_voorstel_medicatieafspraak" resource="Bundle"
            profile="mp-ReplyProposalMedicationAgreement-Bundle"/>
        <nm:map ada="sturen_antwoord_voorstel_medicatieafspraak" resource="Communication"
            profile="mp-ReplyProposalMedicationAgreement"/>
        <nm:map ada="sturen_antwoord_voorstel_verstrekkingsverzoek" resource="Bundle"
            profile="mp-ReplyProposalDispenseRequest-Bundle"/>
        <nm:map ada="sturen_antwoord_voorstel_verstrekkingsverzoek" resource="Communication"
            profile="mp-ReplyProposalDispenseRequest"/>
        <nm:map ada="sturen_voorstel_medicatieafspraak" resource="Bundle"
            profile="mp-ProposalMedicationAgreement-Bundle"/>
        <nm:map ada="sturen_voorstel_verstrekkingsverzoek" resource="Bundle"
        ...
        <nm:map ada="vrijheidsbeperkende_interventie" resource="Procedure"
        ...
        <nm:map ada="zorgverlener" resource="Practitioner"
    ...
    <xd:doc>
        <xd:param name="patientTokensXml">Override optional parameter containing XML document based
            on QualificationTokens.json as used on Github / Touchstone</xd:param>
    </xd:doc>
    <xsl:param name="patientTokensXml"
        select="document('../../ada_2_fhir/fhir/QualificationTokens.xml')"/>

    <xd:doc>
        <xd:desc>Build the metadata for all the FHIR resources that are to be generated from the
            current input.</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>Build the FHIR metadata for the resources that shall result from the transformation
            of the specified ADA instances. This metadata may then be used when building the actual
            FHIR instances for building references and Bundle's. For each generated FHIR instance,
            the FHIR metadata will be stored in an nm:resource element and may contain the following
            elements: <xd:ul>
                ...
                <xd:li>nm:logical-id: the logical id for the FHIR instance, if one is needed (see
                    the <xd:ref name="populateId" type="parameter"/>populateId and <xd:ref
                        name="referencingStrategy" type="parameter"/> parameters). If a logical id
                    is not given by ADA, it will be generated using the best matching template with
                    the _generateId mode.</xd:li>
                <xd:li>nm:full-url: the fullUrl for the resource, based on what is needed for the
                    current referencing strategy (see the <xd:ref name="referencingStrategy"
                        type="parameter"/> parameter).</xd:li>
                <xd:li>nm:ref-url: the url to reference the resource, based on what is needed for
                    the current referencing strategy (see the <xd:ref name="referencingStrategy"
                        type="parameter"/>referencingStrategy parameter).</xd:li>
                <xd:li>nm:reference-display: the display to use when referencing this FHIR resource.
                    The display will be generated by the best matching template with the
                    _generateDisplay mode.</xd:li>
            </xd:ul> Note that for patient and zorgverlener are treated in a special way; if
            multiple patients and zorgverleners are passed with the same identification number but
            with differences in the content, instances for each set of unique content will be
            generated. </xd:desc>
        <xd:param name="in">The ADA instances where the FHIR instances will be generated
            from.</xd:param>
    </xd:doc>
    <xsl:template name="buildFhirMetadata">
        ...
        <xsl:if test="not($referencingStrategy = ('logicalId', 'uuid', 'none'))">
            <xsl:message terminate="yes">Invalid $referencingStrategy. Should be one of 'logicalId',
                'uuid', 'none'</xsl:message>
        </xsl:if>
        ...
        <xsl:for-each
            select="$in[not(.//(@value | @code | @nullFlavor))][not(ends-with(local-name(), '-start'))]">
            <xsl:message>Error: no meaningful content found in <xsl:value-of
    ...
    <xd:doc>
        <xd:desc>Helper template to build the FHIR metadata for a singe ADA instance. See the
            documentation on <xd:ref name="buildFhirMetadata" type="template"/> for more
            information.</xd:desc>
        <xd:param name="in">The ADA instance to generate metadata for.</xd:param>
        <xd:param name="partNumber">The sequence number of the ADA instance being passed in the
            total collection of ADA instances of this kind. This sequence number is needed for ids
            in resources that represent just a part of a zib.</xd:param>
    </xd:doc>
            ...
            <nm:resource profile="{$profile}">
                ...
                <xsl:variable name="logicalId">
                    <xsl:choose>
                        ...
                        <xsl:when test="$in/@logicalId">
                            <xsl:choose>
                                <xsl:when
                                    test="count($ada2resourceType/nm:map[@ada = $adaElement]) gt 1">
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="$referencingStrategy = 'logicalId'">
                        ...
                        <nm:full-url>
                            <xsl:value-of
                                select="concat($serverBaseUriEdit, @resource, '/', $logicalId)"/>
                        </nm:full-url>
                        <nm:ref-url>
    ...
    <xd:doc>
        ...
        <xd:param name="uniqueString">The unique string with which to create a logical id. Optional.
            If not given a uuid will be generated.</xd:param>
    </xd:doc>
    <xsl:template name="generateLogicalId">
        ...
        <xsl:choose>
            <xsl:when
                test="string-length($uniqueString) le $maxLengthFHIRLogicalId - 2 and string-length($uniqueString) gt 0">
                <xsl:value-of
                    select="nf:assure-logicalid-length(nf:assure-logicalid-chars($uniqueString))"/>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xd:doc>
        <xd:desc>Generate a FHIR reference. When there's no input or a reference can't otherwise be
            constructed, no output is generated.</xd:desc>

        <xd:param name="in">The target of the reference as either an ADA instance or an ADA
            reference element. May be omitted if it is the same as the context.</xd:param>
        <xd:param name="profile">The id of the profile that is targeted. This is needed to specify
            which profile is targeted when a single ADA instance is mapped onto multiple FHIR
            profiles. It may be omitted otherwise.</xd:param>
        <xd:param name="wrapIn">Optional element name to wrap the output in. If no output is
            generated, this wrapper will not be generated as well.</xd:param>
    </xd:doc>
    ...
    <xsl:template name="makeReference">
        ...
        <xsl:if test="count($fhirMetadata) = 0">
            <xsl:message terminate="yes">Cannot create reference because $fhirMetadata is empty or
                unknown.</xsl:message>
        </xsl:if>
        ...
        <xsl:variable name="groupKey">
            <xsl:choose>
                <xsl:when
                    test="$in[@datatype = 'reference' and @value] and not(empty(nf:resolveAdaInstance($in, /)))">
            ...
            <xsl:choose>
                <xsl:when test="count($fhirMetadata[nm:group-key = $groupKey]) gt 1">
                    <xsl:if test="string-length($profile) = 0">
                        <xsl:message terminate="yes">makeReference: Duplicate entry found for
                            $groupKey '<xsl:value-of select="$groupKey"/>' in $fhirMetadata, while
                            no $profile was supplied.</xsl:message>
                    </xsl:if>
                    <xsl:if
                        test="not($fhirMetadata[@profile = $profile and nm:group-key = $groupKey])">
                        <xsl:message terminate="yes">makeReference: Duplicate entry found for
                            $groupKey '<xsl:value-of select="$groupKey"/>' in $fhirMetadata, but no
                            valid $profile ('<xsl:value-of select="$profile"/>') was
                            supplied.</xsl:message>
                    </xsl:if>
                    <xsl:if
                        test="count($fhirMetadata[@profile = $profile and nm:group-key = $groupKey]) gt 1">
                        <xsl:message terminate="yes">makeReference: Duplicate entry found for
                            $groupKey '<xsl:value-of select="$groupKey"/>' and $profile
                                '<xsl:value-of select="$profile"/>'in $fhirMetadata.</xsl:message>
                    </xsl:if>
                    <xsl:copy-of
                        select="$fhirMetadata[@profile = $profile and nm:group-key = $groupKey]"/>
                </xsl:when>
                <xsl:otherwise>
        ...
        <xsl:if test="$in and count($element) = 0 and not($identifier)">
            <xsl:call-template name="util:logMessage">
                ...
                <xsl:with-param name="msg">Cannot resolve reference within set of ada-instances:
                        <xsl:value-of select="$groupKey"/></xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        ...
        <xsl:variable name="populatedReference" as="element()*">
            ...
            <xsl:choose>
                <xsl:when
                    test="$referencingStrategy = 'none' and not($element/nm:ref-url) and $identifier">
                    <identifier>
    ...
    <xd:doc>
        <xd:desc>Insert the FHIR resource id element for the specified ADA instance, if an id is
            required (see the <xd:ref name="populateId" type="parameter"/> and <xd:ref
                name="referencingStrategy" type="parameter"/> parameters). </xd:desc>
        <xd:param name="in">The ADA instance that the FHIR resource is generated for.</xd:param>
        <xd:param name="profile">The id of the profile that is being generated from the specified
            ADA instance. This is needed to specify which profile is targeted when a single ADA
            instance results is mapped onto multiple FHIR profiles. It may be omitted
            otherwise.</xd:param>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>Get the FHIR resource id element for the specified ADA instance, if an id is
            available (see the <xd:ref name="populateId" type="parameter"/> and <xd:ref
                name="referencingStrategy" type="parameter"/> parameters). </xd:desc>
        <xd:param name="in">The ADA instance that the FHIR resource is generated for.</xd:param>
        <xd:param name="profile">The id of the profile that is being generated from the specified
            ADA instance. This is needed to specify which profile is targeted when a single ADA
    ...
    <xsl:template name="getLogicalIdFromFhirMetadata">
        ...
        <xsl:if
            test="count($fhirMetadata[nm:group-key = $groupKey]) gt 1 and string-length($profile) = 0">
            <xsl:message terminate="yes">insertId: Duplicate entry found in $fhirMetadata, while no
                $profile was supplied. $groupKey: <xsl:value-of select="$groupKey"/></xsl:message>
        </xsl:if>
        ...
        <xsl:variable name="logicalId">
            <xsl:choose>
                <xsl:when test="count($fhirMetadata[nm:group-key = $groupKey]) gt 1">
                    <xsl:value-of
                        select="$fhirMetadata[@profile = $profile and nm:group-key = $groupKey]/nm:logical-id"
                    />
                </xsl:when>
                <xsl:otherwise>
    ...
    <xd:doc>
        <xd:desc>Insert the FHIR fullUrl element for the specified instance, if one is available
            (see the <xd:ref name="referencingStrategy" type="parameter"/> parameter).</xd:desc>
        <xd:param name="in">The ADA instance that the FHIR resource is generated for.</xd:param>
        <xd:param name="profile">The id of the profile that is being generated from the specified
            ADA instance. This is needed to specify which profile is targeted when a single ADA
            instance results is mapped onto multiple FHIR profiles. It may be omitted
    ...
    <xsl:template name="insertFullUrl">
        ...
        <xsl:if
            test="count($fhirMetadata[nm:group-key = $groupKey]) gt 1 and string-length($profile) = 0">
            <xsl:message terminate="yes">insertFullUrl: Duplicate entry found in $fhirMetadata,
                while no $profile was supplied.</xsl:message>
        </xsl:if>
        ...
        <xsl:variable name="fullUrl">
            <xsl:choose>
                <xsl:when test="count($fhirMetadata[nm:group-key = $groupKey]) gt 1">
                    <xsl:value-of
                        select="$fhirMetadata[@profile = $profile and nm:group-key = $groupKey]/nm:full-url"
                    />
                </xsl:when>
                <xsl:otherwise>
    ...
    <xd:doc>
        <xd:desc>Generic template for generating an id for a FHIR resource. Stylesheets for specific
            zibs can override this with a more specific version.</xd:desc>
        <xd:param name="in">The ADA instance to generate the id for.</xd:param>
        <xd:param name="profile">The id of the profile that is targeted. This is needed to specify
            which profile is targeted when a single ADA instance results is mapped onto multiple
            FHIR profiles. It may be omitted otherwise.</xd:param>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>Generic template for generating a display for use in FHIR references. Stylesheets
            for specific zibs can override this with a more specific version.</xd:desc>
    ...
    <xsl:template match="*" mode="_generateDisplay">
        ...
        <xsl:choose>
            <xsl:when
                test="$in//*[ends-with(local-name(.), '_naam')][@value or @displayName or @originalText]">
                <xsl:value-of
                    select="($in//*[ends-with(local-name(.), '_naam')]/(@displayName, @originalText, @value))[1]"
                />
            </xsl:when>
            <xsl:otherwise>
    ...
    <xd:doc>
        <xd:desc>Function to resolve internal references in an ADA instance. The output is the
            contained ADA instance being referenced or the input node if the input node isn't a
            reference.</xd:desc>
        <xd:param name="in">The ADA instance to resolve.</xd:param>
        <xd:param name="context">The complete ADA instance where the contained ADA instance is
            contained in.</xd:param>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>Returns a string based on requirements for FHIR logicalId
            (https://www.hl7.org/fhir/datatypes.html#id). An underscore is translated to '-'. Any
            other char that is not allowed is simply removed.</xd:desc>
    ...
    <xd:doc>
        <xd:desc>Returns a concatenated string based on input param $logicalId. Only returns a
            string of length max length for FHIR logicalId (64). Because uniqueness is determined
            more by the latter part of $logicalId than by the start (often some sort of prefix), the
            last 64 characters are used.</xd:desc>
        ...
        <xsl:variable name="startingLoc" as="xs:integer">
            <xsl:choose>
                <xsl:when test="$lengthLogicalId gt $maxLengthFHIRLogicalId">
                    <xsl:call-template name="util:logMessage">
                        <xsl:with-param name="msg">We have encountered an id (<xsl:value-of
                                select="$logicalId"/>) longer than 64 characters, we are truncating
                            it, but it should be looked at.</xsl:with-param>
    ...
    <xd:doc>
        <xd:desc>Returns the full profileName for an ada element, based on $urlBaseNictizProfile and
            $ada2resourceType constant. Selects the first one found, which may be wrong if there is
            more than one entry. In which case you should not use this function.</xd:desc>
    ...
    <xsl:function name="nf:get-full-profilename-from-adaelement" as="xs:string?">
        ...
        <xsl:value-of
            select="concat($urlBaseNictizProfile, nf:get-profilename-from-adaelement($adaElement))"
        />
    </xsl:function>

    <xd:doc>
        <xd:desc>Returns the last part of the profileName for an ada element, based on
            $ada2resourceType constant. Selects the first one found, which may be wrong if there is
            more than one entry. In which case you should not use this function.</xd:desc>
        <xd:param name="adaElement">The ada element for which to get the profileName</xd:param>
    </xd:doc>
    ...
    <xd:doc>
        ...
        <xd:param name="adaUnit">The ada alement containing the code/codeSystem in Gstd
            eenheden</xd:param>
    </xd:doc>
    <xsl:template name="_buildMedicationQuantity">
        ...
        <code
            value="{nf:convertGstdBasiseenheid2UCUM($adaUnit[@codeSystem=$oidGStandaardBST902THES2]/@code)}"/>
