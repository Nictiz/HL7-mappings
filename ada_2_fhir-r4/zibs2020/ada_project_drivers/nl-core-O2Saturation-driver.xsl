...
<xsl:stylesheet exclude-result-prefixes="#all"
    ...
    <xsl:template match="/nm:bundle">
        <xsl:apply-templates mode="_doTransform" select="$bundle/o2saturatie"/>
    </xsl:template>
    
    <xsl:template match="//o2saturatie_registratie/o2saturatie">
        <xsl:apply-templates mode="_doTransform" select="."/>
    </xsl:template>
    
    <xsl:template mode="_doTransform" match="o2saturatie">
        ...
        <xsl:call-template name="nl-core-O2Saturation">
