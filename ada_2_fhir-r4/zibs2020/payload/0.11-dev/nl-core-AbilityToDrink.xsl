...
<xsl:stylesheet exclude-result-prefixes="#all"
    ...
    <xsl:variable name="profileNameAbilityToDrinkDrinkingLimitations">nl-core-AbilityToDrink.DrinkingLimitations</xsl:variable>
            ...
            <Observation>
                ...
                <xsl:for-each select="drink_beperkingen">
                    <hasMember>
                        <xsl:call-template name="makeReference">
                            <xsl:with-param name="in" select="."/>
                            <xsl:with-param name="profile" select="$profileNameAbilityToDrinkDrinkingLimitations"/>
                        </xsl:call-template>
                    </hasMember>
                </xsl:for-each>
            </Observation>
    ...
    <xd:doc>
        <xd:desc>Creates an nl-core-AbilityToDrink.DrinkingLimitations instance as an Observation FHIR instance from ADA drink_beperkingen element.</xd:desc>
        <xd:param name="in">ADA element as input. Defaults to self.</xd:param>
        <xd:param name="subject">Optional ADA instance or ADA reference element for the patient.</xd:param>
    </xd:doc>
    <xsl:template match="drink_beperkingen[parent::vermogen_tot_drinken]" name="nl-core-AbilityToDrink.DrinkingLimitations" mode="nl-core-AbilityToDrink.DrinkingLimitations" as="element(f:Observation)?">
        <xsl:param name="in" select="." as="element()?"/>
        <xsl:param name="subject" select="patient/*" as="element()?"/>
    ...
    <xsl:template match="drink_beperkingen[parent::vermogen_tot_drinken]" mode="_generateDisplay">
        <xsl:variable name="parts" as="item()*">
            <xsl:text>drink beperkingen</xsl:text>
            <xsl:if test=".[@value]">
